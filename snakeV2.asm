...
snake_body_print_loop:
	...
	jz 		wall_print 					; if it was zero, move out of here   ##########################################
...
wall_print:
	mov		si, wallS_pos 				; prepare to print wall body
...
walls_print_loop:
	...
	mov		dx, ax						; if not, move the position into dx
	call	move_cursor					; move the cursor there
	mov		al, '#'						; use 'o' as the wall symbol
	call	print_char					; print it
	jmp		walls_print_loop 			; loop
...
move_snake:
	...
	mov		si, wallS_pos 				; prepare to check for wall-collision

;###############################################################################
move_snake2:
	lodsw								; load position of snake body, and increment si
	cmp		ax, bx						; check if head position = body position
	je 		game_over_hit_wall			;
	or		ax, ax						; check if position is 0x0000 (we are done searching)
	jne		move_snake2 				; if not, loop
;###############################################################################
...
drawWalls:
	cmp 	byte [lvl], 1				; Comparar si el nivel actual es 1
	jne 	one							; Si es uno, saltar a la rutina one 
	cmp 	byte [lvl], 2				; Comparar si el nivel actual es 1
	jne 	two							; Si es uno, saltar a la rutina two
	cmp 	byte [lvl], 3				; Comparar si el nivel actual es 1
	jne 	three						; Si es uno, saltar a la rutina two
	mov 	byte [topCtr], 3			; Setear el contador del loop a 3 por default
	jmp 	wallPos						; Saltar a la rutina principal


one:
	mov 	byte [topCtr], 3			; Setear el contador del loop a 5
	jmp 	wallPos						; Saltar a la rutina principal
two:
	mov 	byte [topCtr], 5			; Setear el contador del loop a 5
	jmp 	wallPos						; Saltar a la rutina principal
three:
	mov 	byte [topCtr], 7			; Setear el contador del loop a 7

;########################################################################

wallPos:
	mov		si, wallS_pos 				; Preparar memoria para almacenar las posiciones de los muros

loop1:
	mov 	dx, [topCtr]				;
	cmp 	word [cntrl1], dx			; Comparar topCtr con ctrl1
	jg 		endw						; Salir si cntrl1 es mayor a topctrl
	mov 	bx, 80						; Setear bx como el tope de resolución X
	sub 	bx, [topCtr]				; Restarle el tamaño del contador
	sub 	bx, [topCtr]				; Restarle el tamaño del contador
	call 	rand						; Llamar a la función rand
	add 	dx, [topCtr]				; Sumarle el tamaño del contador
	mov 	word [wall_x_pos], dx		; Guardar el numero resultante de la función rand
	mov 	bx, 25						; Setear bx como el tope de resolución y
	sub 	bx, [topCtr]				; Restarle el tamaño del contador
	sub 	bx, [topCtr]				; Restarle el tamaño del contador
	call 	rand						; Llamar a la función rand
	add 	dx, [topCtr]				; Sumarle el tamaño del contador
	mov 	word [wall_x_pos], dx		; Guardar el numero resultante de la función rand
	mov 	cx, [wallPos]				; Guardar en cx la posición x,y generada
	mov 	[si], cx					; Guardar en la lista de posiciones la posicion generada
	inc 	byte [cntrl1]				; Suma uno al contador
	add		si, 2						; Increment si by 2
	mov 	bx, 4						; Fijar 4 posibles opciones
	call 	rand						;
	cmp 	dx, 1						; Si dx es 1
	je 		plusx						; Aumentamos x
	cmp 	dx, 2						; Si dx es 2
	je 		minx						; Disminuimos x
	cmp 	dx, 3						; Si dx es 3
	je 		plusy						; Aumentamos y
	
miny:
	mov 	dx, [topCtr]
	cmp 	word [cntrl2], dx			; Comparar topCtr con ctrl1;
	jg 		loop1						; salir si cntrl1 es mayor a topctrl
	dec 	byte [wall_y_pos]			; restarle 1 a la posicion x
	mov 	cx, [wallPos]				; guardar en cx la posición x,y generada
	mov 	[si], cx					; Guardar en la lista de posiciones la posicion generada
	add		si, 2						; Increment si by 2
	inc 	byte [cntrl2]				; Aumentar el contador 2
	jmp 	miny			
...
plusy:
	...
	cmp 	word [cntrl2], dx			; Comparar topCtr con ctrl1
	jg 		loop1						; salir si cntrl1 es mayor a topctrl
	inc 	byte [wall_y_pos]			; sumar 1 a la posicion x
	mov 	cx, [wallPos]				; Guardar en cx la posición x,y generada
	mov 	[si], cx					; Guardar en la lista de posiciones la posicion generada
	add		si, 2						; Increment si by 2
	inc 	byte [cntrl2]				; Aumentar el contador 2
...
plusx:
	...
	cmp 	word [cntrl2], dx			; Comparar topCtr con ctrl1
	jg 		loop1						; Salir si cntrl1 es mayor a topctrl
	inc 	byte [wall_x_pos]			; sumar 1 a la posicion y
	mov 	cx, [wallPos]				; guardar en cx la posición x,y generada
	mov 	[si], cx					; Guardar en la lista de posiciones la posicion generada
	add		si, 2						; Increment si by 2
	inc 	byte [cntrl2]				; Aumentar el contador 2
...
minx:
	...
	cmp 	word [cntrl2], dx			; Comparar topCtr con ctrl1
	jg 		loop1						; Salir si cntrl1 es mayor a topctrl
	dec 	byte [wall_x_pos]			; Restarle 1 a la posicion x
	mov 	cx, [wallPos]				; Guardar en cx la posición x,y generada
	mov 	[si], cx					; Guardar en la lista de posiciones la posicion generada
	add		si, 2						; Increment si by 2
	inc 	byte [cntrl2]				; Aumentar el contador 2
...
self_msg db 'yourself', 0xE6 			; f
wall_msg db 'the wall', 0xEC 			; l
score_msg db 'Score:', 0xA0 			; space
instructions db ' Use W (up) A (left) S(down) D(right) to control', 0xA0 ; space

; VARIABLES -------------------------------------------------------------------
lvl db 1								
topCtr db 0								; top para niveles
cntrl1 db 0								; contador loop1
cntrl2 db 0								; contador loop2
